...
function processFile(
  ...
  if (!isSSR && file.filename.endsWith('.html')) { return processHtmlFile(store, file.code, file.filename, processed, seen) }
...
function processChildFiles(
  ...
  if (hasDynamicImport) {
    ...
    // crawl child imports
    for (const imported of importedFiles) { processFile(store, store.state.files[imported], processed, seen, isSSR) }
  }
}
      ...
      if (node.declaration) {
        if (
          ...
          for (const decl of node.declaration.declarations) {
            for (const id of extractIdentifiers(decl.id)) { defineExport(id.name) }
          }
        }
