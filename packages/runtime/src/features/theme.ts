import type { ColorSchemeModes, PinceauMediaQueries, PinceauTheme, Theme, TokensFunctionOptions } from '@pinceau/shared'
import { createTokensHelper, get, normalizeTokens, pathToVarName, resolveReferences, resolveThemeRule, set, walkTokens } from '@pinceau/shared'
...
export function usePinceauThemeSheet(
  ...
  function updateToken(path: string | string[], value: any, mq: PinceauMediaQueries = 'initial') {
    ...
    const resolvedValue = resolveReferences({ value } as any, { }, { $tokens })
  ...
  function createMqRule(mq: PinceauMediaQueries) {
    ...
    const mqRule = resolveThemeRule(mq, '', theme, colorSchemeMode)
