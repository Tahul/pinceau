...
import type { CachedCSSRule, PinceauRuntimePluginOptions, PinceauRuntimeSheet, PinceauThemeSheet, PinceauUidTypes } from './types'

const HYDRATION_SELECTOR = '.phy[--]'

export function useRuntimeSheet({ appId }: { themeSheet?: PinceauThemeSheet } & PinceauRuntimePluginOptions): PinceauRuntimeSheet {
  // Local runtime stylesheet reference
  let sheet: CSSStyleSheet | undefined

  // Local cache for each token CSSRule index
  let cache: { [key: string]: CachedCSSRule } = {}
