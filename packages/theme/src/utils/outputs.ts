...
import { objectPaths } from '@pinceau/core/runtime'
import type { ColorSchemeModes, PinceauMediaQueries } from '../types'
import { flattenTokens, walkTokens } from './tokens'
import { enhanceTokenPaths, stringifyUtils } from './utils-properties'
import { createThemeRule } from './css-rules'
...
export function cssFull(dictionary: Dictionary, _options: Options, _responsiveTokens: any, colorSchemeMode: ColorSchemeModes) {
  ...
  Object.entries(tokens).forEach(
    (pair) => {
      ...
      const content = formattedVariables({ format: 'css', dictionary: { ...dictionary, allTokens: value } as any, outputReferences: true, formatting: { lineSeparator: '', indentation: '', prefix: '' } as any })
      css += createThemeRule({ mq: key, content, theme: tokens, colorSchemeMode })
    },
  )
