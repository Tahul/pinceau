...
import type { PinceauBuildContext, PinceauContext, PinceauOptions, PinceauTheme } from '@pinceau/shared'
import { createTokensHelper, message } from '@pinceau/shared'
...
export function usePinceauContext(options: PinceauOptions): PinceauContext {
  ...
  const buildContext: PinceauBuildContext = {
    ...
    updateViteServer: (server: ViteDevServer) => (viteServer = server),
