...
export const STYLES_EXTENSIONS = ['css', 'sass', 'scss', 'postcss', 'less', 'styl', 'stylus']
export const JS_EXTENSIONS = ['js', 'jsx', 'mjs', 'ts', 'tsx', 'jsx', 'tsx', 'js', 'ts']
export const SUPPORTED_EXTENSIONS = [...STYLES_EXTENSIONS, ...JS_EXTENSIONS, 'vue']
...
export function parsePinceauQuery(id: string) {
  ...
  if (params.has('blockType')) { ret.blockType = params.get('blockType') as PinceauQuery['blockType'] }
  if (params.has('index')) { ret.index = Number(params.get('index')) }
  if (params.has('scoped')) { ret.scoped = String(params.get('scoped')) }
  else if (params.has('lang')) { ret.lang = params.get('lang') as PinceauQuery['lang'] }
  if (params.has('issuerPath')) { ret.issuerPath = params.get('issuerPath') as PinceauQuery['issuerPath'] }
  if (params.has('transformed')) { ret.transformed = true }
  if (STYLES_EXTENSIONS.includes(ret.ext)) { ret.styles = true }
  if (SUPPORTED_EXTENSIONS.includes(ret.ext)) { ret.transformable = true }
  if (langPart) {
