...
export function resolveVariantsProps(variants, isTs: boolean) {
  ...
  Object.entries(variants).forEach(
    ([key, variant]: [string, any]) => {
      ...
      if (isBooleanVariant) {
        prop.type = isTs ? ' [Boolean, Object] as PropType<boolean | { [key in import(\'pinceau\').PinceauMediaQueries]?: boolean }>' : ' [Boolean, Object]'
        prop.default = false
      }
      else {
        const possibleValues = `\'${Object.keys(variant).filter(key => key !== 'options').join('\' | \'')}\'`
        prop.type = isTs ? ` [String, Object] as PropType<${possibleValues} | { [key in import(\'pinceau\').PinceauMediaQueries]?: ${possibleValues} }>` : ' [String, Object]'
